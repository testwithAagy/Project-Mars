...
public class SkillsHooks
{
    ...
    public SkillsHooks(IObjectContainer objectContainer, ScenarioContext scenarioContext)
    {
    ...
    public void BeforeSkillsScenario()
    {
        try
        {
            ...
            var tab = wait.Until(d => d.FindElement(By.XPath("//a[@data-tab='second' and text()='Skills']")));
            tab.Click();
            _test.Log(Status.Pass, "Skills tab opened successfully");

            // Cleanup existing skills before scenario
            LanguagesAndSkillsHelper.CleanupAllTestSkills(_driver);
            _test.Log(Status.Info, "Cleaned up existing test skills before scenario");
    ...
    public void AfterSkillsScenario()
    {
        try
        {
            if (_scenarioContext.ContainsKey("AddedSkills"))
            {
                ...
                {
                    LanguagesAndSkillsHelper.CleanupScenarioSkill(_driver, skill);
                    _test.Log(Status.Info, $"Deleted skill '{skill}' added by this scenario");
                }
            }
        ...
        {
            _driver.Quit();
            _test.Log(Status.Info, "Closed browser for @Skills scenario");
        }
    }
}
